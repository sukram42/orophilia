    ...
    let routes = await Promise.all(
      route_id.map(async (route) => {
        ...
        async function choosemountain(item: mountains) {
          ...
          let positions = {}
          let { data: routesInformation } = await supabase
            .from("routes")
            .select(`*`)
    ...
    <div className="homeContainer">
      ...
      <List
        ...
        renderItem={(item) => (
          <List.Item
            ...
            extra={<img width={128} alt="logo" src={item["wikiimage_url"]} />}
          >
            <List.Item.Meta
      ...
      <div className="map">
        {activeMountain ? (
          <MapContainer
            ref={map}
            center={[activeMountain?.lat, activeMountain?.lon]}
            zoom={13}
          ...
          >
            ...
            {Object.values(routes).map((value, idx) => (
              <div key={idx}>
                {hoveredPosition && (
                  <Popup position={hoveredPosition}>
                    Difficulty T{value.route.hike_difficulty}
                  </Popup>
                )}
                <MapWithPolyline
                  key={idx}
                  width={
                    hoveredRoute && hoveredRoute.route.id === value.route.id
                      ? 10
                      : 5
                  }
                  route={value}
                  coordinates={value.waypoints}
                  color="blue"
                />
              </div>
            ))}
